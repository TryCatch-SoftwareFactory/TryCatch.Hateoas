...
namespace TryCatch.Hateoas.UnitTests.Services
{
    ...
    {
        private readonly FakeLinkService sut;

        public LinksServiceTests()
        {
            this.sut = new FakeLinkService();
        }
        ...
        [Fact]
        public void GetSummaryLinks_Without_Entity()
        {
            // Arrange
            FakeEntity entity = null;

            // Act
            Action actual = () => _ = this.sut.GetSummaryLinks(entity);
        ...
        [Fact]
        public void GetSummaryLinks_With_Invalid_Entity()
        {
            // Arrange
            string entity = null;
        ...
        [Fact]
        public void GetSummaryLinks_With_Valid_Entity()
        {
            // Arrange
            var entity = Given.ValidEntity();
            var expected = Given.ExpectedLinksForSummary();

            // Act
            var actual = this.sut.GetSummaryLinks(entity);
        ...
        [InlineData(0, 10)]
        [InlineData(1, 0)]
        public void GetListResultLinks_With_Invalid_Args(int offset, int limit)
        {
            // Arrange
            var total = 100;

            // Act
            Action actual = () => _ = this.sut.GetListResultLinks(offset, limit, total);
        ...
        [Theory]
        [InlineData(21, 1, 11, 10)]
        [InlineData(11, 0, 1, 10)]
        [InlineData(0, 81, 91, 10)]
        [InlineData(0, 91, 101, 10)]
        public void GetListResultLinks_With_Valid_Args(int offset1, int offset2, int offset, int limit)
        {
            // Arrange
            var total = 100;
            var expected = Given
                .ExpectedLinksForListResult(offset1, offset2, limit)
                .OrderBy(x => x.Rel);

            this.sut.SetFakeQueryParams(offset, limit);

            // Act
            var actual = this.sut.GetListResultLinks(offset, limit, total).OrderBy(x => x.Rel);
